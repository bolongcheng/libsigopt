...
class NumericalTestCase:
    ...
    @staticmethod
    def assert_scalar_is_close(value, truth, tol, abs_tol=DEFAULT_ABS_TOL):
        assert_scalar_is_close(value, truth, tol, abs_tol)

    @staticmethod
    def assert_vector_within_relative(value, truth, tol):
        assert_vector_within_relative(value, truth, tol)

    @staticmethod
    def assert_vector_row_wise_norm_is_close(value, truth, tol, norm=1, abs_tol=DEFAULT_ABS_TOL):
        assert_vector_row_wise_norm_is_close(value, truth, tol, norm, abs_tol)

    @staticmethod
    def assert_vector_within_relative_norm(value, truth, tol, norm=2):
        assert_vector_within_relative_norm(value, truth, tol, norm)

    @staticmethod
    def check_gradient_with_finite_difference(x, func, grad, fd_step, tol, *, use_complex=False):
        check_gradient_with_finite_difference(x, func, grad, fd_step, tol, use_complex=use_complex)
