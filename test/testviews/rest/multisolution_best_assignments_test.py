...
class TestMultisolutionBestAssignments:
    ...
    def test_k_center_clustering_invalid_input_k(self):
        for k in [-1, 0, 3, 50]:
            with pytest.raises(AssertionError):
                k_center_clustering(np.random.randn(3, 2), first_center_index=0, k=k)

    def test_k_center_clustering_invalid_input_first_center_index(self):
        for i in [-2, -1, 10, 27]:
            with pytest.raises(AssertionError):
                k_center_clustering(np.random.randn(10, 2), first_center_index=i, k=3)
    ...
    def test_k_center_clustering_random_points(self, dim, num_points, k):
        ...
        first_center_index = np.random.randint(num_points)
        centers_indices, partition = k_center_clustering(points, first_center_index, k)
        assert centers_indices[0] == first_center_index
        assert len(centers_indices) == k
        assert len(np.unique(partition)) == k
        assert np.all(partition[centers_indices] == range(k))

        distance_matrix_squared = compute_distance_matrix_squared(points[centers_indices], points)
        expected_partition = np.argmin(distance_matrix_squared, axis=0)
        assert np.all(expected_partition == partition)
        for i, c in enumerate(centers_indices):
            assert np.isclose(distance_matrix_squared[i, c], 0.0)

    def test_k_center_clustering_same_points(self):
        num_points = 12
        k = 4
        repeated_point = np.array([[1, 1, 1]])
        points = np.repeat(repeated_point, num_points, axis=0)
        point_far_away = np.array([[9, 9, 9]])
        index_far_away = 3
    ...
    def test_k_center_clustering_same_points_random(self, dim, num_points, k):
        point = np.random.randn(1, dim)
        points = np.repeat(point, num_points, axis=0)
    ...
    def test_k_center_clustering_optimality(self, categorical_dim, num_clusters, num_points_per_clusters, sigma):
        ...
        points = np.copy(cluster_centers)
        for i in range(num_clusters):
            curr_points = np.random.multivariate_normal(
                cluster_centers[i, :],
                sigma * np.eye(dim),
                num_points_per_clusters,
            )
            points = np.concatenate((points, curr_points))
        ...
        optimal_centers_indices = [np.flatnonzero(shuffle_indices == i)[0] for i in range(num_clusters)]
        shuffled_points = points[shuffle_indices, :]
        assert np.allclose(shuffled_points[optimal_centers_indices, :], cluster_centers)

        first_center_index = np.random.randint(shuffled_points.shape[0])
        ...
        assert len(np.unique(partition)) == num_clusters
        assert np.all(partition[centers_indices] == range(num_clusters))

        distance_matrix_squared = compute_distance_matrix_squared(shuffled_points[centers_indices], shuffled_points)
        expected_partition = np.argmin(distance_matrix_squared, axis=0)
        assert np.allclose(expected_partition, partition)

        # k-center cluster must give us an 2-OPT approximation ratio
        largest_radius = np.sqrt(np.max(np.min(distance_matrix_squared, axis=0)))
        optimal_distance_matrix_squared = compute_distance_matrix_squared(
            shuffled_points[optimal_centers_indices],
            shuffled_points,
        )
        optimal_r = np.sqrt(np.max(np.min(optimal_distance_matrix_squared, axis=0)))
