...
def test_pi_search_grad_fail(domain_list, product_of_list_probabilistic_failures_list):
...
def test_pi_search_no_repulsor(domain_list, product_of_list_probabilistic_failures_list):
    for domain, prod_of_pfs in zip(domain_list, product_of_list_probabilistic_failures_list):
        random_distance = np.random.random()
        pi_search = ProbabilityOfImprovementSearch(domain, prod_of_pfs, random_distance)
        ...
        assert_vector_within_relative_norm(pi, product_pv, 1e-15)
        assert np.all((pi >= 0) * (pi <= 1))


def test_pi_search_with_repulsor_points(domain_list, product_of_list_probabilistic_failures_list):
    for domain, prod_of_pfs in zip(domain_list, product_of_list_probabilistic_failures_list):
        random_distance = np.random.random()
...
def test_pi_search_with_evaluate_at_point_lists():
    ...
    assert_vector_within_relative_norm(pi, pos, 1e-15)


@pytest.mark.parametrize("dim", [2, 5])
def test_map_non_categorical_points_to_unit_hypercube(dim):
...
def test_round_one_hot_points_categorical_values_to_target():
...
def test_convert_one_hot_to_search_hypercube_points_distances():
...
@pytest.mark.parametrize("dim_val", [3, 7])
@pytest.mark.parametrize("num_points", [3, 57, 520])
def test_covert_one_hot_to_search_hypercube_points_distances(dim_val, num_points):
    domain = form_random_unconstrained_categorical_domain(dim_val)
    one_hot_domain = domain.one_hot_domain
    one_hot_points = one_hot_domain.generate_quasi_random_points_in_domain(num_points)
    ...
    def check_all_elements_are_present(domain_obj, one_hot_points_arr):
        categorical_component_mappings = domain_obj.get_categorical_component_mappings()
        for categorical_component_mapping in categorical_component_mappings:
            ...
            best_categories = np.argmax(one_hot_points_arr[:, cat_indices], axis=1)
