...
def test_core_functionality(one_hot_domain_list, gaussian_process_list):
    for domain, gp in zip(one_hot_domain_list, gaussian_process_list):
        ...
        assert_vector_within_relative_norm(m, mm, 1e-15)
        assert_vector_within_relative_norm(m, m2, 1e-15)
        assert_vector_within_relative_norm(v, vv, 1e-15)
        # NOTE: this line has 0.1% chance of failing
        assert_vector_within_relative_norm(v, v2, 1e-12)
        assert_vector_within_relative_norm(gm, gm2, 1e-15)
        assert_vector_within_relative_norm(gv, gv2, 1e-15)
...
def test_posterior_sampling(gaussian_process_list):
...
def test_constant_liar_update(one_hot_domain_list, gaussian_process_list):
    for domain, gp in zip(one_hot_domain_list, gaussian_process_list):
