...
def test_gp_sum_properties(gp_sum_setup, idx):
    gp_list, weights = gp_sum_setup["gaussian_process_lists"][idx], gp_sum_setup["weights_lists"][idx]
    gp_sum = GaussianProcessSum(gp_list, weights)
    for gp in gp_list:
...
@pytest.mark.parametrize("idx", range(10))
def test_gp_sum_append_lie_data(gp_sum_setup, idx):
    domain, gp_list, weights = (
        gp_sum_setup["domains"][idx],
        gp_sum_setup["gaussian_process_lists"][idx],
        gp_sum_setup["weights_lists"][idx],
    )
...
@pytest.mark.parametrize("idx", range(10))
def test_gp_sum_best_value_and_location(gp_sum_setup, idx):
    gp_list, weights = gp_sum_setup["gaussian_process_lists"][idx], gp_sum_setup["weights_lists"][idx]
...
@pytest.mark.parametrize("idx", range(10))
def test_gp_sum_by_sampling(gp_sum_setup, idx):
    n_samples = 2111
    num_points = 55
    domain, gp_list, weights = (
        gp_sum_setup["domains"][idx],
...
@pytest.mark.flaky(reruns=2)
@pytest.mark.parametrize("idx", range(10))
def test_gp_sum_draw_posterior_samples_of_points(gp_sum_setup, idx):
    n_samples = 2212
    num_points = 65
...
@pytest.mark.parametrize("idx", range(10))
def test_gp_sum_core_functionality(gp_sum_setup, idx):
    num_points = 51
    domain, gp_list, weights = (
        gp_sum_setup["domains"][idx],
        gp_sum_setup["gaussian_process_lists"][idx],
...
@pytest.mark.parametrize("idx", range(10))
def test_acquisition_functions(gp_sum_setup, build_acquisition_function, acquisition_function_name, idx):
    h = 1e-4
    num_points = 22
    domain, gp_list, weights = (
        gp_sum_setup["domains"][idx],
