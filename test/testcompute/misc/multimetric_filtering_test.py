...
def test_form_convex_combination_weights():
    budget = np.random.randint(50, 100)
    max_points = np.random.randint(10, 50) + budget
    assert all(
        all(
            0 <= w <= 1
            for w in form_convex_combination_weights(MultimetricOptPhase.CONVEX_COMBINATION_RANDOM_SPREAD, n / budget)
        )
...
def test_force_minimum_successful_points():
    margin = 2
    num_points = margin + MULTIMETRIC_MIN_NUM_SUCCESSFUL_POINTS
    points_sampled_values = _create_pareto_frontier_values(num_points)
    ...
    # The best values in optimizing_metric=1 are the last points (see _create_pareto_frontier_values())
...
def test_filter_convex_combination():
    multimetric_info = _form_multimetric_info(MultimetricMethod.CONVEX_COMBINATION)
...
def test_filter_convex_combination_sum_of_gps():
    multimetric_info = _form_multimetric_info(MultimetricMethod.CONVEX_COMBINATION)
    assert isinstance(multimetric_info.params, ConvexCombinationParams)
    assert np.sum(multimetric_info.params.weights) == 1
    points_sampled = form_points_sampled(
...
def test_filter_epsilon_constraint():
    multimetric_info = _form_multimetric_info(MultimetricMethod.EPSILON_CONSTRAINT)
    ...
    points_sampled.values = _create_pareto_frontier_values(num_points)
    (
        modified_points_sampled_points,
        modified_points_sampled_values,
        modified_points_sampled_value_vars,
...
def test_filter_probabilistic_failures():
    num_points = MULTIMETRIC_MIN_NUM_SUCCESSFUL_POINTS * 2
...
def test_filter_epsilon_constraint_all_failures():
    multimetric_info = MultimetricInfo(
        method=MultimetricMethod.EPSILON_CONSTRAINT,
        params=ProbabilisticFailuresParams(
            epsilon=0.1,
    ...
    point_sampled_values = _create_one_optima_values(MULTIMETRIC_MIN_NUM_SUCCESSFUL_POINTS)
...
# NOTE: With the adjusted epsilon bounds, all failures are more likely to occur.
# I have added this test to be sure that we enforce some minimum amount of successful values.
def test_filter_probabilistic_failures_all_failures():
    multimetric_info = MultimetricInfo(
        method=MultimetricMethod.PROBABILISTIC_FAILURES,
        params=ProbabilisticFailuresParams(
            epsilon=0.1,
...
def test_filter_optimize_one_metric():
    multimetric_info = _form_multimetric_info(MultimetricMethod.OPTIMIZING_ONE_METRIC)
...
def test_filter_multimetric_points_sampled(params):
    phase, filter_name, expected_output = params
    multimetric_info = _form_multimetric_info(phase)
    points_sampled = form_points_sampled(
        domain=MIXED_DOMAIN,
        num_sampled=5,
        noise_per_point=1e-5,
...
def test_filter_multimetric_points_sampled_failed():
    multimetric_info = _form_multimetric_info(MultimetricMethod.PROBABILISTIC_FAILURES)
    multimetric_info = MultimetricInfo(method=MultimetricMethod.PROBABILISTIC_FAILURES, params=multimetric_info.params)
...
def test_filter_multimetric_points_sampled_spe_failed():
    multimetric_info = _form_multimetric_info(MultimetricMethod.PROBABILISTIC_FAILURES)
    multimetric_info = MultimetricInfo(method=MultimetricMethod.PROBABILISTIC_FAILURES, params=multimetric_info.params)
    assert multimetric_info.method == MultimetricMethod.PROBABILISTIC_FAILURES
    points_sampled = form_points_sampled(
